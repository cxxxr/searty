            ...
            (let ((inverted-index
                    (resolve-inverted-index database
                                            (list (token-id tok-1)
                                                  (token-id tok-2)
                                                  (token-id tok-3)))))
              (let ((inverted-values (inverted-index-get inverted-index (token-id tok-1))))
                (ok (= 1 (length inverted-values)))
                (let ((inverted-value (elt inverted-values 0)))
                  (ok (equal (inverted-value-document-id inverted-value)
                             (document-id document)))
                  (ok (equal '(0) (inverted-value-positions inverted-value)))))
              (let ((inverted-values (inverted-index-get inverted-index (token-id tok-2))))
                (ok (= 1 (length inverted-values)))
                (let ((inverted-value (elt inverted-values 0)))
                  (ok (equal (inverted-value-document-id inverted-value)
                             (document-id document)))
                  (ok (equal '(1) (inverted-value-positions inverted-value)))))
              (let ((inverted-values (inverted-index-get inverted-index (token-id tok-3))))
                (ok (= 1 (length inverted-values)))
                (let ((inverted-value (elt inverted-values 0)))
                  (ok (equal (inverted-value-document-id inverted-value)
                             (document-id document)))
                  (ok (equal '(2) (inverted-value-positions inverted-value))))))))))))
